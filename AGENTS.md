...
- **Infra & DevOps MCPs (`argocd`, `grafana`, `homarr`, `agent-tools`)**: Agents MUST use `argocd` and `grafana` MCP tools for deployment state and observability, and `agent-tools` primitives via Bifrost (`https://mcp.infra.plexplease.com/mcp`) or local MCP for Kubernetes diagnostics (`k8s_get`, `k8s_logs`, `pvc_status`), in-cluster HTTP routing (`http_request`), and estate entity resolution (`catalog_resolve`). Raw `kubectl` and `docker` shell one-liners should only be used when an equivalent primitive is unavailable or for emergency triage.
...
| `Master-Policy.md` | 2.4 | 2026-09-11 |
