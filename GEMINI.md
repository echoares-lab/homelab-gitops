...
- **Testing & Quality:** Every major component must pass `pytest-testinfra` validation. All code changes must be validated through appropriate linters (`yamllint`, `ansible-lint`, `flake8`, `shellcheck`). A comprehensive **Matrix Test** (`scripts/matrix_test.py`) must be executed and updated to reflect any changes to the orchestrator's logic or infrastructure blueprint.
