...
## 🚀 Key Features
*   **Unified Orchestrator:** A modern Python-based controller (`manage.py`) manages the entire lifecycle: Build, Deploy, Config, Test, Status, and Destroy.
...
Runtime secrets are resolved from `config/secrets.env` through 1Password. The file contains only `op://` references and is safe to commit; secret values stay in the Homelab-GitOps vault.

### 2. Deploy a New Node
Deploy a Photon OS Docker node targeting a specific host:
```bash
python3 manage.py all photon-docker 02 --host esxi-01.mgmt.plexplease.com
```
...
Check the managed fleet without making changes:
```bash
