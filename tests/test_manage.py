...
from manage import (
    identify_vm, resolve_playbook, PLAYBOOK_MAP,
    BUILD_TARGETS, app
)
...
def test_identify_vm_direct_match(mock_run_cmd, mock_status):
    ...
    mock_run_cmd.assert_called_once_with(
        "tofu workspace list", cwd="tofu", capture=True)
...
def test_identify_vm_ip_match(mock_run_cmd, mock_status):
    ...
    assert any(
        "192.168.1.50" in c and "govc find" in c for c in govc_call_args
    ), (
        f"Expected a govc find call with the IP address; "
        f"calls were: {govc_call_args}"
    )
...
def test_resolve_playbook_from_profiles(profile, expected_playbook):
    """resolve_playbook reads profile YAML and maps the tag correctly."""
    ...
    assert playbook == expected_playbook, (
        f"Profile '{profile}' → expected '{expected_playbook}', "
        f"got '{playbook}'"
    )
...
def test_playbook_map_values_are_tuples(tag, expected):
    """Every PLAYBOOK_MAP entry is (playbook_filename, required_vars)."""
    playbook, req_vars = expected
    assert playbook.endswith(
        ".yml"), f"Tag '{tag}': playbook should be a .yml filename"
    assert isinstance(
        req_vars, list), f"Tag '{tag}': required_vars should be a list"
...
def test_build_targets_structure():
    ...
    for target, (hcl, label) in BUILD_TARGETS.items():
        assert hcl.endswith(
            ".pkr.hcl"), f"Target '{target}': expected .pkr.hcl path"
        assert isinstance(
            label, str) and label, (
            f"Target '{target}': label must be non-empty string"
        )
...
def test_alias_registered(alias, canonical):
    ...
    assert alias in names, (
        f"Alias '{alias}' (for '{canonical}') not registered in app"
    )
...
def test_canonical_commands_still_registered():
    ...
    names = _registered_command_names()
    for canonical in [
        "build", "lint", "deploy", "config", "test", "destroy", "all",
        "create-profile", "edit-profile", "create-role", "create-play"
    ]:
        assert canonical in names, (
            f"Canonical command '{canonical}' missing from app"
        )
...
def test_profile_specs_are_positive(profile_path):
    ...
    for field in ("cpu", "ram_gb", "disk_size_gb"):
        ...
        assert isinstance(val, int) and val > 0, (
            f"{os.path.basename(profile_path)}: vm_specs.{field} "
            f"must be a positive int, got {val!r}"
        )
