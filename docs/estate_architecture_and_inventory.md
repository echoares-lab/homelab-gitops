...
| **DISC-10** | **Reverse-proxied targets left pointing at the tombstoned HOMELAB host (`10.10.10.30`)** | kubernetes_clusters[k3s-01].reverse_proxied_external_services listed `openbao.infra.plexplease.com` at `10.10.10.30` port 8201, `plex.infra.plexplease.com` at `10.10.10.30` port 32400 and `sabnzbd.infra.plexplease.com` at `10.10.10.30` port 8080. `10.10.10.30` is VM HOMELAB (vm-7030), tombstoned 2026-09-14 in homelab-gitops PR 176 (commit 0b32512). | Read-only checks 2026-09-14. ping `10.10.10.30`: 2 packets transmitted, 0 received, 100% packet loss. `kubectl get ingress -A`: plex (namespace plex, host `plex.infra.plexplease.com`, class traefik, address `10.10.10.50`, age 54d) and sabnzbd (namespace media, host `sabnzbd.infra.plexplease.com`, address `10.10.10.50`, age 8d) are in-cluster Ingresses, backed by EndpointSlices plex-vhpcp (pod `10.42.0.74`) and sabnzbd-qjtmh (pod `10.42.0.21`) -- pod addresses, so neither is reverse-proxied off-cluster at all. `getent hosts`: `openbao.infra.plexplease.com` and `openbao.plexplease.com` both resolve to `10.10.10.40`, and EndpointSlice openbao in namespace openbao-external points at `10.10.10.40`. There is no in-cluster openbao Ingress. `10.10.10.40` is VM vault-01 (qemu-140 on pm-01), role "OpenBao / HashiCorp Vault secrets backend". | Resolved 2026-09-14 on that evidence: `openbao.infra.plexplease.com` retargeted to `10.10.10.40`; plex moved out of reverse_proxied_external_services into the in-cluster deployment list with namespace plex and port 32400; the sabnzbd reverse-proxy entry deleted because sabnzbd was already recorded as an in-cluster deployment in namespace media. scripts/catalog_lint.py rule stale_target now fails CI on any reverse-proxied target whose address is undeclared or belongs to a tombstoned entity, so this class of drift cannot survive a pull request again. |
